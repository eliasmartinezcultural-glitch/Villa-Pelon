...
function renderChapter(){
 ...
 story.innerHTML=`<div class="card"><div class="top"><span>VILLA PELÓN · V1.5</span><span>AVENTURA ${chapter+1}/${chapters.length}</span></div><div class="timeline">${chapters.map((_,i)=>`<i class="dot ${i<=chapter?"on":""}"></i>`).join("")}</div><div class="era">🌟 ${c.era}</div><h2 class="title">${c.title}</h2><p class="narrative">${c.text}</p><div class="memory"><span>🧠 LO QUE YA SABÉS</span><strong>${memory}</strong></div><div class="mission"><span>🎯 MISIÓN</span><strong>Encontrá 2 pistas y descubrí qué tienen en común.</strong><div class="case-status"><i id="caseFill"></i><span id="caseText">0/2 pistas</span></div></div><div class="evidence-grid">${c.evidence.map((e,i)=>`<button class="evidence-card" data-index="${i}"><span class="icon">${e[0]}</span><span class="evidence-name">${e[1]}</span><span class="evidence-text">${e[2]}</span><span class="inspect">👀 MIRAR</span></button>`).join("")}</div><div id="questionBox" class="question-box" style="display:none"><div class="question-label">🕵️ TU ELECCIÓN</div><div class="question">${c.question}</div><div class="answers">${c.answers.map((a,i)=>`<button class="answer" data-index="${i}">${a}</button>`).join("")}</div><div id="feedback" class="feedback"></div><button id="next" class="primary next" style="display:none" id="next">${chapter<chapters.length-1?"SEGUIR LA HUELLA →":"CONTARLE LA HISTORIA →"}</button></div><div class="journal"><span>📖 CUADERNO</span><strong id="journalCount">${discoveries.length} pistas · ${connections} conexiones</strong></div></div>`
}
